...
//        System.out.println("The error for test data is " + testError);
        ...
        JavaRDD<Tuple2<Integer, String>> moviesTitlesRDD = moviesInfoDF.javaRDD().map(
                (Function<Row, Tuple2<Integer, String>>) row -> new Tuple2<Integer, String>(row.getInt(0), row.getString(1)));

        JavaPairRDD<Integer, Iterable<Tuple2<Integer, Double>>> moviesIdWithRatingsRDD = ratingsInfoDF.javaRDD().map(
                row -> new Tuple2<>(row.getInt(1), row.getDouble(2))
        ).groupBy(tuple -> tuple._1);

        JavaRDD<Tuple2<Integer, Tuple2<Integer, Double>>> movieIdWithAvgRatingsRDD = moviesIdWithRatingsRDD.map(idRatingsTuple -> {
            int noOfRatings = 0;
        ...
        JavaRDD<Tuple2<Integer, Integer>> movieRatingCountsRDD = movieIdWithAvgRatingsRDD.map(tuple -> new Tuple2<>(tuple._1, tuple._2._1));
        ...
        JavaRDD<Rating> newUserRatingsRDD = jsc.parallelize(Arrays.asList(newUserRatings)).map(arr -> new Rating(arr[0], arr[1], arr[2]));
        ...
        List newUserRatedMovieIds = Arrays.asList(newUserRatings).parallelStream().map(arr -> arr[1]).collect(Collectors.toList());
        JavaRDD<Row> newUserUnratedMovies = moviesInfoDF.toJavaRDD().filter(
                row -> !newUserRatedMovieIds.contains(row.getInt(0))
        );
        JavaRDD<Tuple2<Object, Object>> x1 = newUserUnratedMovies.map((Function<Row, Tuple2<Object, Object>>) row -> new Tuple2<Object, Object>(0, row.getInt(0)));

        RDD<Rating> newUserRecommendationsRDD = newModel.predict(JavaRDD.toRDD(x1));
        JavaRDD<Tuple2<Integer, Double>> newUserRecommendationsRatingsRDD = newUserRecommendationsRDD.toJavaRDD().map(
                (Function<Rating, Tuple2<Integer, Double>>) r -> new Tuple2<>(r.product(), r.rating())
        );
        JavaPairRDD<Integer, Tuple2<Tuple2<Double, String>, Integer>> newUserRecommendationsRatingTitleAndCountRDD = JavaPairRDD.fromJavaRDD(newUserRecommendationsRatingsRDD).
